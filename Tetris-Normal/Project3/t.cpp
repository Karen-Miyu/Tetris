...
*
*   Game developed by Marc Palau and Ramon Santamaria
*
...
void InitGame(void)
{
    ...
    for (int i = 0; i < Row; i++)
    {
        for (int j = 0; j < Col; j++)
        {
            if ((j == Col - 1) || (i == 0) || (i == Row - 1)) grid[i][j] = BLOCK;
            else grid[i][j] = EMPTY;
        }
    }
        ...
        if (!pause)
        {
            ...
            else
            {
                ...
                if (fadeLineCounter >= FADING_TIME)
                {
                    ...
                    lines += deletedLines; //Serve para acompanhar o progresso do usuário
                }
            }
        }
    }
    ...
    if (!gameOver)
    {
        ...
        for (int j = 0; j < Col; j++)
        {
            for (int i = 0; i < Row; i++)
            {
                ...
                {
                    DrawRectangle(offset.x, offset.y, SQUARE_SIZE, SQUARE_SIZE, DARKBLUE);
                    offset.x += SQUARE_SIZE;
                }
        ...
        DrawText("INCOMING:", offset.x, offset.y - 100, 10, LIGHTGRAY);
        DrawText(TextFormat("LINES:      %04i", lines), offset.x, offset.y + 20, 10, LIGHTGRAY);
        DrawText("Presione 'P' para pausar", offset.x, offset.y + 40, 11, LIGHTGRAY);
        DrawText("Presione 'E' para sair", offset.x, offset.y + 60, 11, LIGHTGRAY);

        if (pause) DrawText("GAME PAUSED", screenWidth / 2 - MeasureText("GAME PAUSED", 54) / 2, screenHeight / 2 - 40, 40, WHITE);
