...
void DrawGame(void)
{
    ...
    // Alterando fonte de texto
    Font fontPropolish = LoadFont("PropolishRufftu-BLLyd.ttf");

    Vector2 position = { GetScreenWidth() / 2 - MeasureText("PRESSIONE [ENTER] PARA JOGAR DE NOVO", 32) / 2, screenHeight / 2 - 40 };
    ...
    if (!gameOver)
    {
        ...
        Vector2 position1 = { offset.x, offset.y - 100 };
        Vector2 position2 = { offset.x, offset.y + 20 };
        Vector2 position3 = { offset.x, offset.y + 40 };
        Vector2 position4 = { offset.x, offset.y + 60 };
        Vector2 position5 = { screenWidth / 2 - MeasureText("JOGO PAUSADO", 47) / 2, screenHeight / 2 - 40 };

        DrawTextEx(fontPropolish, "PROXIMO:", position1, 15, 1, LIGHTGRAY);
        DrawTextEx(fontPropolish, TextFormat("LINHAS:      %04i", linesDelted), position2, 20, 1, LIGHTGRAY);
        DrawTextEx(fontPropolish, "'P' PARA PAUSAR", position3, 20, 1, LIGHTGRAY);
        DrawTextEx(fontPropolish, "'E' PARA SAIR", position4, 20, 1, LIGHTGRAY);

        if (pause) {
            // Retângulo semi-transparente sobre o wallpaper existente
            DrawRectangle(0, 0, GetScreenWidth(), GetScreenHeight(), Fade(BLACK, 0.5f));
            DrawTextEx(fontPropolish, "JOGO PAUSADO", position5, 50, 1, WHITE);
        }
    }
    else DrawTextEx(fontPropolish, "PRESSIONE [ENTER] PARA JOGAR DE NOVO", position, 50, 1, WHITE);
    EndDrawing();
    UnloadFont(fontPropolish);
}
